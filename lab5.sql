-- Practica 2 Nivel Fisico - Copia Autoracle
-- Joaquin Terrasa, a 1 de Abril de 2020

-- [1] Acceder a la máquina virtual
-- [2] Comprobar los servicios que se están ejecutando
PRINT('Se ejecutan los servicios "OracleServiceORCL", "OracleOraDB18Home1TNSListener" y 3 mas relacionados con Oracle')

-- [3] Ejecutar SqlDeveloper (como sysdba)
-- [4] Obtener los directorios de los ficheros de datos--> v$datafile
SELECT NAME FROM V$DATAFILE;

PRINT('Obtenemos 5 rutas (5 datafiles):
C:\USERS\ALUMNOS\ORACLE\ORADATA\ORCL\SYSTEM01.DBF
C:\USERS\ALUMNOS\ORACLE\ORADATA\ORCL\SYSAUX01.DBF
C:\USERS\ALUMNOS\ORACLE\ORADATA\ORCL\UNDOTBS01.DBF
C:\USERS\ALUMNOS\ORACLE_INSTALACION\DATABASE\AUTORACLE.DBF
C:\USERS\ALUMNOS\ORACLE\ORADATA\ORCL\USERS01.DBF')

-- [5] Si no existe, crear un tablespace denominado TS_AUTORACLE con 16 Megas, con incrementos de 200K y un máximo de 128 M. El fichero de datos se debe denominar 'autoracle.dbf' y debe residir el mismo directorio que los demás datafiles.
PRINT('Dado que existe un tablespace con ese nombre, vamos a modificarlo paara cumplir con lo especificado')
PRINT('https://docs.oracle.com/cd/B28359_01/server.111/b28310/dfiles003.htm')

ALTER DATABASE DATAFILE 'C:\USERS\ALUMNOS\ORACLE_INSTALACION\DATABASE\AUTORACLE.DBF'
    RESIZE 16M;
...
-- [6] Crear un usuario denominado AUTORACLE (si existe de alguna práctica anterior, borrarlo con la instrucción 'drop user AUTORACLE CASCADE') con permiso para conectarse y cuota ilimitada en el tablespace TS_AUTORACLE, que será su tablespace por defecto. Darle permiso para conectarse, crear tablas, crear vistas, crear vistas materializadas.
CREATE USER AUTORACLE IDENTIFIED BY autoracle
    DEFAULT TABLESPACE TS_AUTORACLE
    QUOTA UNLIMITED ON TS_AUTORACLE;

GRANT CREATE SESSION, CONNECT TO AUTORACLE;
GRANT CREATE TABLE, CREATE MATERIALIZED VIEW, CREATE VIEW TO AUTORACLE;


-- [7] Conéctate a la base de datos APOLO en afrodita.lcc.uma.es con tu usuario de prácticas (UBDXXX).
-- [8] Vamos a generar el script con todas las sentencias SQL necesarias para copiar los datos de Autoracle de Apolo en el usuario AUTORACLE local de la MV. Aunque existen otras formas de hacerlo, vamos a realizarlo con la opción "Herramientas -> Exportación" de Bases de datos de Sqldeveloper.
-- [9] Después de pulsar siguiente, en la ventana "Especificar Objetos" hay que buscar el usuario "Autoracle" de Apolo. Para ello, pulsar el botón donde pone "Mas" y seleccionar el esquema "AUTORACLE".
-- [10] Al especificar datos, pulsar siguiente y a continuación, Terminar

-- [11] Crear una conexión para el usuario AUTORACLE local.
PRINT('Conectado como AUTORACLE en la MV')

-- [12] Ejecutar el script generado con la conexión AUTORACLE local.
--------------------------------------------------------
